...
1.  **Enrich Biodiversity**\
    1-1. **Healthy Vegetation Density**: using NDVI calculation, the areas with the most active photosynthesis process can be identified, which indicates the areas have a better vegetation health condition for enriching biodiversity.\
    1-2. **Natural Land Use Density**: the natural land use along the streams provide legislative and administrative base for nature-based interventions; indicate a more natural environment.\
    1-3. **Species Richness**: different types of natural land use indicate different degree of species richness which provides different condition for interventions aiming for enriching biodiversity.\
    1-4. **Nuisance from Industries**: industries in general cause nuisance (noise, pollution, odor, etc) which hinder biodiversity development.\
    1-5. **Nuisance from Traffic**: infrastructure such as highways cause nuisance such as noise and pollution, thus hindering the biodiversity in the area.\
    1-6. **Nuisance from Urban Barriers**: roads with a high maxspeed become dangerous urban barriers preventing animals from migrating and thus hindering the biodiversity.\

2.  **Promote Climate Adaptation**\
    2-1. **Risk of Flooding**: there have been major flooding in Poznan during the last few years, and the areas with higher flooding risk are more in need of interventions that promote climate adaptations.\
    ...
    2-4. **River Sinuosity**: the higher sinuosity of a stream indicate that it is in a more natural form and helps to diverse flows and habitats, thus helping to promote climate adaptation.\
...
3.  **Improve Quality of Life**\
    ...
    3-2. **Urbanized Land Use Density**: the urbanized land use along the streams provide legislative and administrative base for community-oriented interventions and it also indicates a higher urbanization degree.\
    3-3. **People Attraction Density**: if the area already contains some attractions for people, it is more likely to become a more active space in the future.\
    3-4. **Stream Navigability**: the areas with denser roads have better accessibility; it also indicates higher degree of urbanization.\
    3-5. **Local Centrality**: the local integration analysis shows where the most vital local centers are and the areas with higher local centrality will be more suitable for community-oriented interventions to be carried out.\
    ...
    3-7. **Density Indicator (FSI/GSI/OSR)**: those figures indicate urbanization degree of a certain area.\
    3-8. **Attraction Betweenness**: attraction betweenness analysis shows which roads are more frequently chosen leading to certain attractions (in this case streams), and the areas with more frequently chosen roads when heading towards streams have better accessibility, thus more suitable for community-oriented solutions.\
...
*general description of workflow, each step what is the operation and what is the goal*

#### Chosen Fields

*please move the parts below here*
2-4. Terrain Slope:\
...
![](report_files/libs/imgs/2-1.jpeg)\
Processing: ***NDVI --\> Threshold \> 0.2 --\> Zonal Statistics --\> Mean --\> Normalization***\
![](report_files/libs/imgs/2-2_vegetation_health_index.jpeg)\
Accuracy/Reflection: the CIR map come from Copernicus Sentinel 2, the threshold of 0.2 is based on research from Gandhi, 2015
...
![](report_files/libs/imgs/2-4_GreensLandusemap.png)\
Accuracy/Reflection:
...
![](report_files/libs/imgs/2-6_Species_Richness_index.jpg)\
Accuracy/Reflection:
...
![](report_files/libs/imgs/2-8_nuisence by industries.png)\
Accuracy/Reflection:

1-5. Nuisance from Traffic\
![](report_files/libs/imgs/2-9_road_typologies_data.jpg)\
Processing: ***road data --\> Threshold: maxspeed \>= 40km/h --\> Intersection and Join --\> Sum --\> Normalization***\
![](report_files/libs/imgs/2-10_traffic_noise_disturbance_index.jpg)\
Accuracy/Reflection:

1-6. Nuisance from Urban Barrier\
![](report_files/libs/imgs/2-11_urban_barrier_data.jpeg)\
Processing: ***road data --\> Threshold: maxspeed \>= 40km/h --\> Intersection  --\> Sum of length in each cell --\> Normalization***\
![](report_files/libs/imgs/2-12_urban_barrier_index.jpeg)\
Accuracy/Reflection: roads with maxspeed over 40km/h will be more dangerous for animals to cross, thus becoming barriers hindering biodiversity in the area
...
![](report_files/libs/imgs/4-4_Landusemap_urbanisation.png)\
Accuracy/Reflection:
...
**Variables**  Discussed variables: Channel morphology Sinuosity Floodplain width Open water surface Land use cover Impervious Vegetation Landscape Green space connectivity Mobility Slow mobility accessibility Building morphology Floor area ratio
...
```
=======
Typology construction based on an 'Requirements by Priority': Must-have, Should-have, Could-have, and wish list. 
...
>>>>>>> Stashed changes
Our typology creation thus follows two narratives: Which locations are in need of intervention, and which locations are ready for intervention. By answering these questions, we will be able to fill in the 'requirements by priority' school of thought and develop a targeted approach for Urban Stream Restoration. 
...
Our variables: 
  RISK ATTRRIBUTES (require intervention):
  Heat_risk (aggregated hot nights and UHI max)
  flood_risk_rp50
  surface_impermeability
  vegetation_health_index
  
  BENEFIT ATTRIBUTES (ready for intervention):
  terrain_slope
  density_indicators (any can be used, OSR as most logical?) # THIS DOES NOT PROVIDE GOOD RESULTS
...
Cluster description:
1 - (45 features) Cluster situated predominantly around the river, high fr (since this is generally the only area that has potential for flooding), and otherwise generally strong positive results (hr, is, hv, ab). Due to the high ts, the sn is also on the lower end.
2 - (335 features) Cluster represents strongly vegetated areas, with a high hv and low hr, is is also strongly correlated here. Relatively high ts results in both low ab and sn. 
3 - (271 features) HIGHEST hr, LOWEST hv. Broadly urban areas with low fr and sn since not very close to water bodies. High is and low ab, meaning that while it is a high risk area, the impact of interventions would be low. 
4 - (24 features) Generally quite median scores across the board. With a high ab and relatively high sn, the impact of interventions would be felt quite highly, and there is generally a large amount of 'promenade' area. 
5 - (61 features) Highly scoring in all RISK attributes (hr, fr, is, inverse(hv)). Relatively high ab, and very high sn suggest high impact of interventions which would greatly impact the streams themselves. 
```{r}
