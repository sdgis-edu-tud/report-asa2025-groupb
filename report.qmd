...
pal <- colorNumeric(
  palette = "Greens", 
  domain = eb_index_wgs84$LQ_aggregated_index,
  reverse = FALSE,
...
leaflet(data = eb_index_wgs84) %>%
  ...
  addPolygons(
    ...
    label = ~paste("Research Question 1: Aggregated Index:", round(LQ_aggregated_index, 2))
  ) %>%
  addPolylines(
    data = stream_wgs84,
    color = "black",
...
```{r}
# Read the combined result
lq_index <- st_read("files/promote_climate_adaptation/CA_aggregated.gpkg", quiet = TRUE)
stream <- st_read("files/waterway.gpkg", quiet = TRUE)
...
leaflet(data = lq_index_wgs84) %>%
  ...
  addPolygons(
    ...
    label = ~paste("Research Question 2: Aggregated Index:", round(LQ_aggregated_index, 2))
  ) %>%
  addPolylines(
    data = stream_wgs84,
    color = "black",
